...
if(!class_exists('MH_Query')){
	...
	class MH_Query {
		 ...
		 *
		 * @return MH_Query this for chaining.
		 * @global object $wpdb
		 *
		 * @since 1.0.0
		 *
		 ...
		 *
		 * @return MH_Query The current query builder.
		 */
		 ...
		 * @since 1.0.1
		 */
		public function pluckOneColumnArray($column) {
			$this->select = $column;
			$arr = [];
			$rows = $this->get();
			if($rows){
		 ...
		 *
		 */
		public function get( $output = ARRAY_A, $row_map = null, $calc_rows = false ) {
			global $wpdb;
		 ...
		 */
		public function one( $output = ARRAY_A ) {
			global $wpdb;
		 ...
		 *
		 */
		public function rows_found($page, $per_page) {
			global $wpdb;
		...
		public function update( $data ) {
			...
			foreach ( $data as $column => $value ) {
				...
				if(is_numeric( $value ) && !in_array($column, $this->string_columns)){
					$fields[] = "`$column` = " . $wpdb->prepare(strpos($value,'.') !== false ? '%s' : '%d', $value ); 
				}
				else if(in_array($column, $this->json_columns)){
					$fields[] = "`$column` = " . "'$value'";
				}
				else
					$fields[] = "`$column` = " . $wpdb->prepare( '%s', $value );
			}
		...
		private function _get_url($full = true){
	        if($full)
